/* =========================================================
   NexHire — Candidate Signup Controller
   ========================================================= */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    submitBtn.innerHTML = `<i data-lucide="loader-2" class="spin-icon"></i> Registering...`;
    if (window.lucide) window.lucide.createIcons();

    try {
      const data = await window.AuthApi.registerCandidate({
        firstName,
        lastName,
        email,
        phone,
      ...
      // Save returned JWT & profile info
      window.NexAuth.setSession(data);

      showMessage("Registration successful! Redirecting...", false);
      window.NexUtils.showToast("Account created! Redirecting to dashboard...", "success");
      ...
      submitBtn.innerHTML = `<span>Create Candidate Account</span> <i data-lucide="arrow-right"></i>`;
      if (window.lucide) window.lucide.createIcons();
    }
  });
});
